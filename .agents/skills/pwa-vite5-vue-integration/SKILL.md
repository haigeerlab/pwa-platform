...
4. Record exact files changed, commands and browser observations inside the host repository. Separate passed checks from unrun checks. If package release, private repository access, a valid CSS build, deterministic assets or deployment facts are missing, state the specific blocker; never substitute the platform's isolated fixture results for host acceptance. When reporting back to the public platform repository, share only a redacted outcome and reproducible platform-level issue; keep host names, paths, domains, configurations, logs and screenshots in the private host repository unless its owner explicitly approves disclosure.
