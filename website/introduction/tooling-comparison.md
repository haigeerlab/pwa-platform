...
**核查日期：2026-09-26。**“本平台”列描述 npm 已发布的 `0.1.0-beta.2`（`next`）；`latest` 仍为 beta.1，需固定 beta.2 才能获得 Vite 5 与默认更新提示。外部产品依据文末官方资料核对。手机上可横向滑动表格。

## 构建与 Service Worker 工具

| 维度 | 本平台 `0.1.0-beta.2` | vite-plugin-pwa | Workbox |
| --- | --- | --- | --- |
...
以下能力已经随 `0.1.0-beta.2` 发布；固定 beta.1 或省略版本号安装，仍不会得到它们：

| 能力 | 用户可感知的结果 | 当前状态 |
| --- | --- | --- |
| Vite 5 兼容与 `vite dev` 虚拟模块 | 可在 Vite 5 + Vue 3.4 项目中进行接入与本地开发 | beta.2 已发布；独立消费项目从 npm 安装并构建通过 |
| Vue／React 可选更新提示 UI | 业务显式挂载后显示更新卡片；位置、文案、表面与按钮色值可配置 | beta.2 已发布；真实业务项目仍需自行验收 |
