...
**核查日期：2026-09-26。**“本平台”列只描述 npm 已发布的 `0.1.0-beta.1`；下文另列当前工作区已实现、尚未发包的变化。外部产品依据文末官方资料核对。手机上可横向滑动表格。

## 构建与 Service Worker 工具

| 维度 | 本平台 `0.1.0-beta.1` | vite-plugin-pwa | Workbox |
| --- | --- | --- | --- |
| 接入位置 | Vite 构建插件 + Vue／React 页面绑定；目前公开接入范围为 Vite 8 | Vite 插件，可选生成或自行编写 Service Worker | 可组合的 Service Worker、页面和构建模块；开发者自行组织应用接入 |
...
| 更新交互 | 页面绑定暴露等待状态、检查和接管方法；业务自行展示提示，并决定何时刷新 | 注册模块有待更新回调与更新函数；业务在回调里展示提示，也可选择自动更新 | `workbox-window` 提供等待等生命周期事件；业务组织接管与提示 |
...
| [Progressier](https://progressier.com/)（商业服务） | 安装页与安装组件、可视化 manifest、Push 发送界面和 API、统计 | 适合希望由服务提供安装推广和通知运营界面的团队；本平台公开 beta 包不提供这些托管服务。平台工作区的 Push 包也尚未公开发布。 |

上表只描述官方资料明确展示的产品方向，不能据此推断它们没有其他能力。商业服务的功能、套餐和价格会变化，采用前以其当时的官方说明为准。

## 已实现但尚未进入 npm 包

当前工作区在 `0.1.0-beta.1` 之后增加了以下内容，**现在安装 npm beta 包不会得到它们**：

| 工作区变化 | 用户可感知的结果 | 发布前状态 |
| --- | --- | --- |
| Vite 5 兼容与 `vite dev` 虚拟模块 | 可在 Vite 5 + Vue 3.4 项目中进行接入与本地开发 | 已做独立消费方构建和测试；需随下一版 npm 包发布 |
| Vue／React 可选更新提示 UI | 业务显式挂载后显示更新卡片；位置、文案、表面与按钮色值可配置 | 已在工作区实现和本地浏览器核对；需随下一版 npm 包发布 |

更新分为两个动作：新的 Service Worker 先安装并准备资源，用户确认后由它接管；**刷新页面**才会重新运行当前页面的应用代码。当前默认卡片在接管后提供“刷新页面”按钮，不会自行刷新。若业务有未保存表单，可接管页面动作并安排刷新时机。详细交互见[安装与更新](/guide/updates)。网页本身没有刷新却继续运行旧 JavaScript 的问题，在未使用 PWA 的单页应用中也可能存在；接入 PWA 后仍需由业务处理页面状态和刷新体验。
