...
本页用于判断是否值得继续评估某项能力。入口灾备包已公开；其余下列可选包仍是**工作区私有包**，不要把它们加进外部业务项目的安装命令。
...
| 入口灾备 | <code>@pwa-platform/entry-resilience</code> | 域名迁移或现有入口不可达 | npm 0.1.0；旧应用壳须能从缓存启动，业务提前交入清单；用户确认跳转，新地址重新登录 |
