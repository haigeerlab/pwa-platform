...
/** Package specifiers this package's production closure may use. */
const ALLOWED_PACKAGES = ["@pwa-platform/client-runtime", "react"];
...
describe("package manifest", () => {
  ...
  it("depends on client-runtime only, with react as the single peer", () => {
    expect(manifest["dependencies"]).toEqual({ "@pwa-platform/client-runtime": "workspace:*" });
    // react-dom is deliberately absent: this package only uses APIs exported by `react` itself, and the renderer
    // is the application's choice (spec: dependencies).
    expect(manifest["peerDependencies"]).toEqual({ react: "^19.2.0" });
  });

  it("keeps react-dom out of the dev dependencies too", () => {
    // Checked by membership rather than as an exact set: T3 adds @pwa-platform/vue here for the parity suite, and
    // an exact match would turn that expected change into a spurious failure.
    const dev = manifest["devDependencies"] as Record<string, string>;
    expect(Object.keys(dev)).toContain("react");
    expect(Object.keys(dev)).toContain("@types/react");
    expect(Object.keys(dev)).not.toContain("react-dom");
  });
...
describe("dependency boundaries", () => {
  ...
  it("keeps the import closure within client-runtime and react", () => {
  ...
  it("never reaches the vue package", () => {
    // T3 adds @pwa-platform/vue as a devDependency for the parity suite. It must never leak into the production
    // closure: an application using only React would then be made to install Vue, which is exactly what the
    // "two independent packages" decision exists to prevent. This guard is written before that dependency exists.
    const offenders = [...files].flatMap(([file, text]) =>
      specifiers(text)
        .filter((specifier) => specifier.startsWith("@pwa-platform/vue"))
  ...
  it("never reaches the vite adapter", () => {
    // `pwa()` is Node build-time code; importing it would drag the build pipeline into the browser bundle.
    const offenders = [...files].flatMap(([file, text]) =>
      specifiers(text)
  ...
  it("never reaches react-dom", () => {
    const offenders = [...files].flatMap(([file, text]) =>
      specifiers(text)
        .filter((specifier) => specifier === "react-dom" || specifier.startsWith("react-dom/"))
        .map((specifier) => `${file}: ${specifier}`),
    );
    expect(offenders).toEqual([]);
  });
