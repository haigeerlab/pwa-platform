...
公开站点：[pwa-platform-docs.pages.dev](https://pwa-platform-docs.pages.dev/)；源仓库为 [`haigeerlab/pwa-platform`](https://github.com/haigeerlab/pwa-platform)。Cloudflare Pages 项目名为 `pwa-platform-docs`，连接本仓库；日常合并到 `main` 不会自动部署文档。首次部署记录：提交 `16c715cfdfa3279cdd3e663b39c7a7f792c4a6ba`，部署 ID `1e9cbb2d-1661-4f89-b3f7-695272038cb8`。
...
为减少部署次数，文档与功能改动可以照常合并到 `main`；只有决定发布文档时，才从已通过 CI 的 `main` 提交建立版本分支，例如 `docs/v2026.09.25`。每个版本分支固定指向一次发布候选；后续修改先进入 `main`，再建立新的版本分支。发布顺序如下：

1. 在本地运行 `pnpm docs:build`、`pnpm docs:preview`，检查首页、接入页、搜索、代码复制和 404。合并到 `main` 后确认 GitHub Actions 的文档构建通过。
2. 确认 Pages 项目的 **Enable automatic production branch deployments** 为关闭、**Preview branch** 为 **None**；把 `main` 的目标提交推到远端，再从该提交创建并推送唯一的 `docs/v...` 版本分支。推送这两个分支均不应触发 Pages 部署。
