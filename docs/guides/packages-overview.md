...
9 个包已发布到 npm，当前版本是 **`0.1.0-beta.1`**（2026-09-24；首批 `0.1.0-beta.0` 于 2026-09-20 发布）。这是 **beta 版，不代表稳定生产版**。

```bash
# Vue 项目
pnpm add @pwa-platform/vue@0.1.0-beta.1
pnpm add -D @pwa-platform/vite@0.1.0-beta.1
...
pnpm add -D @pwa-platform/contracts@0.1.0-beta.1
```

注意事项：

- **请写明版本号。** npm 首次发布时自动生成了 `latest` 标签，目前它同样指向 beta 版。所以不带版本号安装也会装上 beta 版，但这不代表它已经是稳定版。写明版本号还能避免以后被静默升级。
- **peer 依赖需要项目自带**：`vite` `^8.0.0`、`vue` `^3.4.0`、`react` `^19.2.0`。React 绑定不要求 `react-dom`，渲染器由你的应用决定。
- 其余内部包（core、sw-runtime 等）由包管理器自动解析，不需要手动安装。
- 能装上不等于能上线：业务应用上线前仍要通过自己的生产发布检查。发布记录见 [2026-09-20 发布记录](../../tasks/package-distribution/release-2026-09-20.md)。
...
- **不做什么**：不提供任何界面，更新提示、安装按钮都由你自己画；不在更新后自动刷新页面；不重试、不轮询（定时检查需要你通过 `updateCheck` 显式开启）。
- **入口**：
...
- **它是什么**：当前域名迁移或不可达时，为已安装的应用展示一个经过签名、需要用户确认的备用入口。
- **替你解决**：换域名后，已安装的 PWA 不会直接失联。
...
5. **目前只保证 Chrome 桌面端。** v1 按桌面端通道发布（[ADR-0030](../adr/0030-desktop-release-channel.md)），保证范围是 Chrome 桌面端的当前版和上一个稳定版。**Chrome Android 未经验证、不做任何保证**：安装、离线、推送，乃至基础网页体验，都没有在 Android 上检查过。平台无法阻止 Android 用户访问你的应用，所以如果你的用户主要在手机上，请把这一点告诉产品负责人，不要对外宣称支持 Android。平台首次按 `desktop+android` 通道发布之后，这一条才会改变。
