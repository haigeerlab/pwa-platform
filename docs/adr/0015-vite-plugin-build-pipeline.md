...
上述同次构建校验不能证明两次构建的指纹 URL 与内容稳定。首个项目使用的 `vite-plugin-bundle-obfuscator@1.8.0` 在未设置 `options.seed` 时，实测同名 JS 会生成不同字节，而 worker 因指纹 URL 未变也不变。该业务接入须设置固定 `seed` 并重复构建比对；不改变 `fingerprinted` 与 `revision: null` 的契约，因为该契约还用于旧资产保留及不可变资源响应头验证。若重复构建仍不稳定，不能发布该业务接入。
