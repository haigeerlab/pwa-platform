...
**隔离 Vue 3.4 构建链：** Vite 5.0.0、Vue 3.4.0、Vue Router 4.2.4、Vuex 4.0.2、Vue/JSX 插件、压缩和混淆步骤与平台 Vue 绑定能构建。`vue-tsc` 1.8.11 在 TypeScript 5.2.2 的 `moduleResolution: "Node"` 下通过；使用 `Bundler` 时，夹具所用 Vuex 4.0.2 的旧 `exports`/声明组合会报找不到类型。Chrome 中，不运行 CSS 清理的构建保留组件样式；生产页面注册并离线加载，开发页面显示 Vue 内容且无 worker。其他宿主的依赖和 `tsconfig` 须单独验证。

**混淆插件的发布条件：** 隔离夹具未固定随机种子时，同一源码连续两次构建产生同名但不同字节的 JS，worker 字节却不变。这是漏更新风险；把所有文件改成非指纹虽能改变 worker，却会破坏旧资产保留和缓存响应头判定，故没有采用。固定种子后，相同源码连续构建的 JS 与 worker 哈希一致；改动夹具源码后，JS 文件名与 worker 哈希都变化。采用随机构建步骤的宿主须在真实配置中固定输入并重复构建比对；当前只验证了隔离夹具。
...
**尚未完成的宿主验收：** 隔离夹具不含真实宿主源码、旧 worker、注册调用、部署域名和响应头，不能宣称已在任何业务仓库接入或上线。夹具中的 CSS 清理插件曾产出无效 CSS，即使移除平台插件也会出现；其他宿主若遇到同类问题须在自身构建链中定位。默认更新 UI、竞品对照与 npm 新版发布不属于本修订的已交付项。
