...
## OP7：iPhone 离线页联网恢复补强（2026-09-26）

正式版真机验收发现 iOS 27 主屏幕应用断网时 `navigator.onLine` 仍为 true，恢复网络后没有 `online` 事件；原页面文案所承诺的自动重载没有发生。先在同一真机验证当前 worker 脚本的同源 `HEAD`：断网超时、联网返回 200，且平台对非 GET 请求按现有规则透传。

**实施：** 只改默认离线页的固定内联脚本，在可见时每 10 秒及收到 `online` 事件时探测控制它的公开 worker 脚本；3 秒超时，仅 2xx 刷新，保留手动重试。Android 真机发现 Wi-Fi 刚开启时先收到 `online`，而联网请求仍失败；因此该事件也须经过探测，不能直接刷新。无 controller、失败或非 2xx 时保持页面。同步规格、接入说明和 CSP `connect-src 'self'` 前提，不更改 worker 路由、身份、缓存策略或离线页配置 API。

**验证：** 单元覆盖固定脚本和 CSP 哈希；真实浏览器分别强制吞掉 `online`、在实际离线时提前派发 `online`，均不误刷新，恢复网络由 HEAD 探测自动刷新；公开 React/Vue iPhone 和 Android 安装窗口实体断网→恢复网络复测。最后重跑受影响包与正式候选门禁。
