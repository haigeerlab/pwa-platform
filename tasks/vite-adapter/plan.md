...
### V5-4：复刻首个项目构建链并做浏览器验收

**范围：** 用已提供的 `vite.config.ts` 和锁文件复刻关键链路：Vue/JSX、PurgeCSS、Terser、`static/assets` 和 `enforce: "post"` 的混淆。移除旧 `VitePWA` 后加入平台插件；实际项目源码可用时再跑一次项目本身，不以最小夹具替代它。
