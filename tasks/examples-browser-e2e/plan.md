...
- **"typecheck 通过"在 T1–T8 中对本机之外不成立。** 本包从未声明 `@types/node`，主工作区的 TypeScript 一路向上解析到了仓库之外的 `/Users/vilin/node_modules/@types/node`（22.13.4）；干净 worktree 里 typecheck 以 `TS2688` 失败。已补声明 `@types/node@24.13.4`（项目所有者批准）。
