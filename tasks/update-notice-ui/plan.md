...
7. **隔离槽真实更新**：仅在 `drill` 构建中显式挂载两侧默认提示，保留本地／`main` 示例的既有横幅；本地核对构建、页面和旧套件。完成现场用量核对后，依次发布 v1、v2 并在 Android/iPhone 上核对等待、更新、接管与显式刷新；记录部署 ID、worker 状态、页面版本和失败项。云端门禁未通过时停在本地，不把模拟客户端结果写成真实更新通过。
