...
首个拟接入应用 `example-vite-app` 的锁文件固定了 Vite 5.0.0、Vue 3.4.0、`@vitejs/plugin-vue` 5.0.0、`@vitejs/plugin-vue-jsx` 3.1.0、TypeScript 5.2.2 与 pnpm 8.6.5。其 Vite 配置使用 `base: "/"`、`assetsDir: "static/assets"`、PurgeCSS、Terser 和构建后混淆插件。应用所有者决定移除现有 `vite-plugin-pwa` 配置并改用本平台；旧 PWA 从未上线，因此无需迁移已安装用户或线上旧缓存。
...
- 对 Vite 已赋予指纹文件名的 JS/CSS，业务构建插件必须保证同一输入生成相同字节；否则同一 URL 会发布不同内容，现有预缓存、旧资产保留和 `Cache-Control` 契约无法同时成立。首个项目的 `vite-plugin-bundle-obfuscator@1.8.0` 须设置固定 `options.seed`，并在相同源码上连续构建两次比较文件名与 SHA-256。没有这项证据不得宣称该项目可安全更新。
...
4. 首个项目的混淆步骤使用固定种子后，连续两次同源构建的同名 JS/CSS 字节相同；改动源码后文件名或预缓存修订值变化，新 worker 在浏览器中等待确认。无种子配置的失败对照需记录。
5. 接入文档写清迁移步骤、`base` 与资源规则、开发/生产行为差异、Node.js 最低小版本和真实业务部署的未验证项；发布状态只在包实际发布后更新。

### 业务侧实施交接（2026-09-26）

项目所有者明确内部仓库不提供给平台侧操作。本仓库只交付可复制的 Vite 5 + Vue 3.4 接入手册与项目级 AI Skill；真实业务源码修改、构建和部署验收由业务仓库中的执行者完成。手册与 Skill 必须区分当前源码与已发布 `0.1.0-beta.1`，保留 `static/assets`、PurgeCSS、混淆随机种子和构建时钟等已知检查点，并明确不得把隔离夹具通过写成业务项目通过。

### 开放问题

- 首个业务项目 Node 22 的精确小版本、项目源码与部署响应头尚未取得；真实项目验收与声明最低 Node 小版本需以这些事实为准。
